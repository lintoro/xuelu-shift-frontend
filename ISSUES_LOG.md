...
### 📌 [需求 #028] 全新 5 大排班營運規則改寫覆蓋、分組聯防打烊與營運支援空班警示

- **來源反饋**：現場主管最新營運排班指令，要求全面覆蓋舊有規則，實裝現場精準用人算理：
  1. 清潔組排班僅 A 班，其它使用加班方式處理（這點保持）。
  2. 本鋪/小鋪 一組，GAGOO/餐飲 一組，閉店班每組可以只排 1 位。
  3. 服務台 只排 D、B 班，需要其它班別手動調整。
  4. 以 D 班去排平日，假日延長時間插入 C 班，手動排 B 班。
  5. 營運支援平日 1A 1B，假日 1A 1C，營運支援優先以符合法規及劃休安排，若有空班秀警示手動修正。
- **現狀分析與算理改寫**：
  - **規則 1（清潔組）**：清潔組同仁無論平日或假日，排班剛性鎖定為 A 班（08:30~17:30），其他時段以現場加班方式處理，免除假日 C 班告警。
  - **規則 2（分組聯防打烊）**：
    - 定義門市選品組（本鋪 `ST_MAIN_SHOP` + 小鋪 `ST_SUB_SHOP`）與美食餐飲組（GAGOO `ST_Gagoo` + 餐飲 `ST_DINING`）。
    - 假日延時營業時，每組只指派 1 名 C 班打烊清帳，同組其餘同仁排 D 班。合規檢核器同步改為分組合併計算，只要組內有 1 人 C 班即合格。
  - **規則 3（服務台限定）**：服務台自動排班僅排 D 班（第 1 位）與 B 班（第 2 位及之後），不自動排 A 或 C 班，其他班別由主管手動微調。
  - **規則 4（全館基調）**：平日全館營業站點基調以 D 班排定（工時佔比逾 80%），假日延長時間插入 C 班，需要 B 班由主管手動微調。
  - **規則 5（營運支援與空班警示）**：
    - 營運支援人力平日需求 1A 1B，假日需求 1A 1C。
    - 優先符合勞基法 7 休 1、11 小時班距與同仁劃休。
    - 若排休後人力不足造成空班，合規驗證器精確產出 `ADMIN_SHIFT_DEFICIT`，排班大表頂部即時顯示琥珀色警示橫幅，並提供快捷日期按鈕引導主管手動微調補班。
- **影響檔案清單**：
  - `src/engine/schedulerEngine.js`（改寫派工迴圈、導入分組閉店追蹤器 `groupClosingTracker`、改寫 `selectShiftForStation`、支援營運支援 1A1B/1A1C 配置）
  - `src/engine/complianceValidator.js`（更新假日 C 班分組聯防檢驗、新增營運支援空班缺工 `ADMIN_SHIFT_DEFICIT` 檢核）
  - `src/components/ScheduleTable.jsx`（新增營運支援空班警示橫幅，點擊日期直接鎖定並引導主管微調修正）
  - `scratch/test_v351_new_shift_rules.mjs`（全新 5 大排班規則自動化單元測試腳本）
- **驗證成果**：
  - 單元測試 `scratch/test_v351_new_shift_rules.mjs` 100% 通過：
    - 清潔組 100% 僅排 A 班（0 異常）。
    - 假日延時門市組與美食組 C 班人數每組均 <= 1（0 異常）。
    - 服務台出勤班別 100% 僅排 D 與 B 班（0 異常）。
    - 平日營業站點以 D 班為基調排定（D 班佔比高達 82.5%）。
    - 營運支援空班精確識別並標記供介面呈現。
  - 圖標與建置測試：
    - `scratch/check_lucide_imports.mjs` 通過。
